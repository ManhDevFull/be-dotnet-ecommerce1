...
namespace dotnet.Repository
{
  public class OrderRepository : IOrderRepository
  {
    ...
    SELECT
        ...
        -- Tính tổng tiền từ bảng orderdetail
        COALESCE(
            SUM(
                od.quantity * (
                    CASE
                        WHEN d.typediscount = 1 THEN 
                            ROUND(v.price * (1 - COALESCE(d.discount, 0)::NUMERIC / 100.0))
                        WHEN d.typediscount = 2 THEN 
                            v.price - COALESCE(d.discount, 0)
                        ELSE 
                            v.price
                    END
                )
        ...
        0
        )::BIGINT AS ""TotalPriceAfterDiscount""
        
    FROM orders o
        JOIN orderdetail od ON o.id = od._id
        JOIN variant v ON od.""_idVariant"" = v.id
